...
func (t Time) FillNumber(dec *MyDecimal) {
	...
	if err != nil {
		terror.Log(errors.Errorf("[fatal] never happen because we've control the format!"))
	}
...
func (t Time) Convert(sc *StatementContext, tp uint8) (Time, error) {
...
func (t Time) CompareString(sc *StatementContext, str string) (int, error) {
...
func (t Time) RoundFrac(sc *StatementContext, fsp int8) (Time, error) {
...
func (t *Time) check(sc *StatementContext) error {
...
func (t *Time) Sub(sc *StatementContext, t1 *Time) Duration {
...
func (t *Time) Add(sc *StatementContext, d Duration) (Time, error) {
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-literals.html.
func parseDatetime(sc *StatementContext, str string, fsp int8, isFloat bool) (Time, error) {
	...
	case 1:
		...
		if truncatedOrIncorrect && sc != nil {
			terror.Log(ErrTruncatedWrongVal.GenWithStackByArgs("datetime", str))
			err = nil
		}
	case 2:
...
func (d Duration) ConvertToTime(sc *StatementContext, tp uint8) (Time, error) {
...
func (d Duration) CompareString(sc *StatementContext, str string) (int, error) {
	// use MaxFsp to parse the string
...
// See http://dev.mysql.com/doc/refman/5.7/en/fractional-seconds.html
func ParseDuration(sc *StatementContext, str string, fsp int8) (Duration, error) {
	var (
...
func getTime(sc *StatementContext, num int64, tp byte) (Time, error) {
	s1 := num / 1000000
	s2 := num - s1*1000000
...
func parseDateTimeFromNum(sc *StatementContext, num int64) (Time, error) {
	t := ZeroDate
	// Check zero.
	if num == 0 {
		return t, nil
...
func ParseTime(sc *StatementContext, str string, tp byte, fsp int8) (Time, error) {
...
func ParseTimeFromFloatString(sc *StatementContext, str string, tp byte, fsp int8) (Time, error) {
...
func parseTime(sc *StatementContext, str string, tp byte, fsp int8, isFloat bool) (Time, error) {
	fsp, err := CheckFsp(int(fsp))
	if err != nil {
...
func ParseDatetime(sc *StatementContext, str string) (Time, error) {
...
func ParseTimeFromNum(sc *StatementContext, num int64, tp byte, fsp int8) (Time, error) {
...
func checkTimestampType(sc *StatementContext, t MysqlTime) error {
...
func (t *Time) StrToDate(sc *StatementContext, date, format string) bool {
