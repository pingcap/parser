...
func truncateStr(str string, flen int) string {
	if flen != types.UnspecifiedLength && len(str) > flen {
		str = str[:flen]
	}
	return str
}
...
func ConvertIntToUint(sc *StatementContext, val int64, upperBound uint64, tp byte) (uint64, error) {
...
func ConvertFloatToUint(sc *StatementContext, fval float64, upperBound uint64, tp byte) (uint64, error) {
	val := RoundFloat(fval)
	if val < 0 {
...
func convertDecimalStrToUint(sc *StatementContext, str string, upperBound uint64, tp byte) (uint64, error) {
...
func ConvertDecimalToUint(sc *StatementContext, d *MyDecimal, upperBound uint64, tp byte) (uint64, error) {
...
func StrToInt(sc *StatementContext, str string) (int64, error) {
...
func StrToUint(sc *StatementContext, str string) (uint64, error) {
	str = strings.TrimSpace(str)
	validPrefix, err := getValidIntPrefix(sc, str)
	if validPrefix[0] == '+' {
...
func getValidIntPrefix(sc *StatementContext, str string) (string, error) {
...
func floatStrToIntStr(sc *StatementContext, validFloat string, oriStr string) (intStr string, _ error) {
	var dotIdx = -1
	var eIdx = -1
...
func StrToFloat(sc *StatementContext, str string) (float64, error) {
	str = strings.TrimSpace(str)
...
func ConvertJSONToInt(sc *StatementContext, j BinaryJSON, unsigned bool) (int64, error) {
	switch j.TypeCode {
...
func ConvertJSONToFloat(sc *StatementContext, j BinaryJSON) (float64, error) {
	switch j.TypeCode {
	case TypeCodeObject, TypeCodeArray:
		return 0, nil
	case TypeCodeLiteral:
...
func getValidFloatPrefix(sc *StatementContext, s string) (valid string, err error) {
