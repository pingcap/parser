...
func (ts *testDDLSuite) TestAlterTableSpecRestore(c *C) {
	testCases := []NodeRestoreTestCase{
		...
		{"ADD PARTITION IF NOT EXISTS (PARTITION `p5` VALUES LESS THAN (2010) COMMENT = 'xxx')", "ADD PARTITION IF NOT EXISTS (PARTITION `p5` VALUES LESS THAN (2010) COMMENT = 'xxx')"},
		{"coalesce partition 3", "COALESCE PARTITION 3"},
		{"drop partition p1", "DROP PARTITION `p1`"},
		{"drop partition if exists p1", "DROP PARTITION IF EXISTS `p1`"},
