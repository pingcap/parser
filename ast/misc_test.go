...
func (ts *testMiscSuite) TestExplainStmtRestore(c *C) {
	testCases := []NodeRestoreTestCase{
		...
		{"EXPLAIN FORMAT = 'row' SELECT 1", "EXPLAIN SELECT 1"},
	}
	extractNodeFunc := func(node Node) Node {
		return node.(*ExplainStmt)
	}
	RunNodeRestoreTest(c, testCases, "%s", extractNodeFunc)
}

func (ts *testMiscSuite) TestPrepareStmtRestore(c *C) {
	testCases := []NodeRestoreTestCase{
		{"PREPARE pname FROM 'SELECT ?'", "PREPARE `pname` FROM 'SELECT ?'"},
		{"PREPARE pname FROM @test", "PREPARE `pname` FROM @`test`"},
	}
	extractNodeFunc := func(node Node) Node {
		return node.(*PrepareStmt)
	}
	RunNodeRestoreTest(c, testCases, "%s", extractNodeFunc)
}

func (ts *testMiscSuite) TestDeallocateStmtRestore(c *C) {
	testCases := []NodeRestoreTestCase{
		{"DEALLOCATE PREPARE test", "DEALLOCATE PREPARE `test`"},
	}
	extractNodeFunc := func(node Node) Node {
		return node.(*DeallocateStmt)
	}
	RunNodeRestoreTest(c, testCases, "%s", extractNodeFunc)
}

func (ts *testMiscSuite) TestExecuteStmtRestore(c *C) {
	testCases := []NodeRestoreTestCase{
		{"EXECUTE test", "EXECUTE `test`"},
		{"EXECUTE test USING @var1,@var2", "EXECUTE `test` USING @`var1`,@`var2`"},
	}
	extractNodeFunc := func(node Node) Node {
		return node.(*ExecuteStmt)
	}
	RunNodeRestoreTest(c, testCases, "%s", extractNodeFunc)
}
