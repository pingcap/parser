...
func (s *testSQLGeneratorSuite) TestGenerator(c *C) {
	...
	s.runRandomTest(c, source, "t1", expect)
	s.runEnumTest(c, source, "t1", expect)
}
...
func (s *testSQLGeneratorSuite) runRandomTest(c *C, source string, productionName string, expect []string) {
	...
	for i := 0; i < 1000; i++ {
		generated := strings.Trim(sqlRandIter.Next(), " ")
		c.Assert(isExist(generated, expect), IsTrue)
	}
}
...
func (s *testSQLGeneratorSuite) runEnumTest(c *C, source string, productionName string, expect []string) {
	p := yacc_parser.Parse(yacc_parser.Tokenize(bufio.NewReader(bytes.NewBuffer([]byte(source)))))
