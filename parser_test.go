...
func (s *testParserSuite) TestPrivilege(c *C) {
	table := []testCase{
		...
		{`ALTER USER IF EXISTS CURRENT_USER default role none`, true, "ALTER USER IF EXISTS CURRENT_USER DEFAULT ROLE NONE"},
		{`ALTER USER IF EXISTS CURRENT_USER default role all`, true, "ALTER USER IF EXISTS CURRENT_USER DEFAULT ROLE ALL"},
		{`ALTER USER 'root'@'localhost' default role r1`, true, "ALTER USER `root`@`localhost` DEFAULT ROLE `r1`@`%`"},
		{`ALTER USER 'root'@'localhost', u1@localhost default role r1`, false, ""},
