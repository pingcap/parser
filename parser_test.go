...
func (s *testParserSuite) TestDMLStmt(c *C) {
	table := []testCase{
		...
		{"select * from t into outfile '/tmp/result.txt' union select * from t;", false, ""},
		{"select * from t into outfile '/tmp/result.txt' union (select * from t);", false, ""},
		{"select * from t union select * from t into outfile 'x' union select * from t", false, ""},
		{"select * from t union (select * from t into outfile 'x') union select * from t", false, ""},
		{"select * from t union ((select * from t into outfile 'x') union select * from t)", false, ""},
		{"trace select * from t union ((select * from t into outfile 'x') union select * from t)", false, ""},
		{"explain select * from t union ((select * from t into outfile 'x') union select * from t)", false, ""},
		{"create binding for select * from t union ((select * from t into outfile 'x') union select * from t)", false, ""},
		{"create view a as select * from t1 into outfile 'x';", false, ""},
		{"create view a as (select * from t1 into outfile 'x');", false, ""},
		{"create table a as select * from t1 into outfile 'x';", false, ""},
		{"insert into t select * from t into outfile 'x';", false, ""},
		{"insert into t (a) select a from t into outfile 'x';", false, ""},
		{"select * from (select * from t into outfile 'x') as t;", false, ""},
		{"select * from t where (select a from t into outfile 'x') > 1;", false, ""},
		{"select * from t union (select * from t into outfile '/tmp/result.txt');", true, "SELECT * FROM `t` UNION (SELECT * FROM `t` INTO OUTFILE '/tmp/result.txt')"},
		{"select * from t union select * from t into outfile '/tmp/result.txt';", true, "SELECT * FROM `t` UNION SELECT * FROM `t` INTO OUTFILE '/tmp/result.txt'"},
