...
func (s *testParserSuite) TestDDL(c *C) {
	table := []testCase{
		...
		//{"CREATE TABLE foo (name CHAR(50) BINARY);", true, "CREATE TABLE `foo`(`name` CHAR(50) BINARY);"}, //不支持 binary 逻辑
...
(PARTITION part0 VALUES LESS THAN (1) COMMENT = '1月份' ENGINE = InnoDB,
 ...
 PARTITION part11 VALUES LESS THAN (12) COMMENT = '12月份' ENGINE = InnoDB) */ ;`, true, ""},
		...
		// for check clause
		{"create table t (c1 bool, c2 bool, check (c1 in (0, 1)), check (c2 in (0, 1)))", true, ""}, // check有问题
		{"CREATE TABLE Customer (SD integer CHECK (SD > 0), First_Name varchar(30));", true, ""},    // check有问题
