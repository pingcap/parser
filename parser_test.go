...
func (s *testParserSuite) TestErrorMsg(c *C) {
	...
	c.Assert(err.Error(), Equals, "[DB:parser:1818] Supports only YEAR or YEAR(4) column")

	_, _, err = parser.Parse("select ifnull(a,0) & ifnull(a,0) like '55' ESCAPE '\\\\a' from t;", "", "")
	c.Assert(err.Error(), Equals, "[DB:parser:1210] Incorrect arguments to ESCAPE")

	_, _, err = parser.Parse("load data infile 'aaa' into table aaa FIELDS  Enclosed by '\\\\b';", "", "")
	c.Assert(err.Error(), Equals, "[DB:parser:1083] Field separator argument is not what is expected; check the manual")
	...
	c.Assert(err.Error(), Equals, "[DB:parser:1115] Unknown character set: ''")
	...
	c.Assert(err.Error(), Equals, "[DB:parser:1115] Unknown character set: 'SOME_INVALID_CHARSET'")

	_, _, err = parser.Parse("ALTER DATABASE t COLLATE = ''", "", "")
	c.Assert(err.Error(), Equals, "[DB:ddl:1273] Unknown collation: ''")

	_, _, err = parser.Parse("ALTER SCHEMA t COLLATE = 'SOME_INVALID_COLLATION'", "", "")
	c.Assert(err.Error(), Equals, "[DB:ddl:1273] Unknown collation: 'SOME_INVALID_COLLATION'")
	...
	c.Assert(err.Error(), Equals, "[DB:parser:1525] Incorrect argument (should be Y or N) value: ''")
	...
	c.Assert(err.Error(), Equals, "[DB:ddl:1492] For RANGE partitions each partition must be defined")

	_, _, err = parser.Parse("alter table t partition by list FIELDS(a)", "", "")
	c.Assert(err.Error(), Equals, "[DB:ddl:1492] For LIST partitions each partition must be defined")
	...
	c.Assert(err.Error(), Equals, "[DB:parser:1801] Unknown LOCK type 'first'")

	_, _, err = parser.Parse("alter table t lock = start", "", "")
	c.Assert(err.Error(), Equals, "[DB:parser:1801] Unknown LOCK type 'start'")

	_, _, err = parser.Parse("alter table t lock = commit", "", "")
	c.Assert(err.Error(), Equals, "[DB:parser:1801] Unknown LOCK type 'commit'")

	_, _, err = parser.Parse("alter table t lock = binlog", "", "")
	c.Assert(err.Error(), Equals, "[DB:parser:1801] Unknown LOCK type 'binlog'")

	_, _, err = parser.Parse("alter table t lock = randomStr123", "", "")
	c.Assert(err.Error(), Equals, "[DB:parser:1801] Unknown LOCK type 'randomStr123'")

	_, _, err = parser.Parse("create table t (a longtext unicode)", "", "")
	c.Assert(err.Error(), Equals, "[DB:parser:1115] Unknown character set: 'ucs2'")
	...
	c.Assert(err.Error(), Equals, "[DB:ddl:1273] Unknown collation: 'some_unknown_collation'")
}
