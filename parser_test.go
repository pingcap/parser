...
func (s *testParserSuite) TestBuiltin(c *C) {
	table := []testCase{
		...
		{"SELECT *, CAST(data AS CHAR CHARSET utf8) FROM t;", true, "SELECT *,CAST(`data` AS CHAR CHARSET utf8) FROM `t`"},
		...
		// for issue 224
		{`SELECT CAST('test collated returns' AS CHAR CHARSET utf8) COLLATE utf8_bin;`, true, "SELECT CAST('test collated returns' AS CHAR CHARSET utf8)"},
